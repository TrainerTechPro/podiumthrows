...
function SessionDetail({ session }: { session: SelfLoggedSession }) {
    ...
    <div className="px-4 pb-4 pt-1 space-y-4 animate-in slide-in-from-top-2 duration-200">
      ...
      {(session.sleepQuality || session.sorenessLevel || session.energyLevel) && (
        <div className="flex gap-6 text-xs">
          <div>
            ...
            <div className="mt-0.5"><ScaleDots value={session.sleepQuality} /></div>
          </div>
          <div>
            <span className="text-muted">Soreness</span>
            <div className="mt-0.5"><ScaleDots value={session.sorenessLevel} /></div>
          </div>
          <div>
            <span className="text-muted">Energy</span>
            <div className="mt-0.5"><ScaleDots value={session.energyLevel} /></div>
          </div>
        </div>
      )}

      {/* Technique + Mental focus */}
      {(session.techniqueRating || session.mentalFocus) && (
        <div className="flex gap-6 text-xs">
          {session.techniqueRating && (
            <div>
              <span className="text-muted">Technique</span>
              <div className="mt-0.5"><ScaleDots value={session.techniqueRating} /></div>
            </div>
          )}
          {session.mentalFocus && (
            <div>
              <span className="text-muted">Mental Focus</span>
              <div className="mt-0.5"><ScaleDots value={session.mentalFocus} /></div>
            </div>
          )}
        </div>
      )}
      ...
      {(session.bestPart || session.improvementArea || session.notes) && (
        <div className="space-y-1 text-xs">
          {session.bestPart && (
            <p><span className="text-muted">Went well:</span> <span className="text-[var(--foreground)]">{session.bestPart}</span></p>
          )}
          {session.improvementArea && (
            <p><span className="text-muted">Needs work:</span> <span className="text-[var(--foreground)]">{session.improvementArea}</span></p>
          )}
          {session.notes && (
            <p><span className="text-muted">Notes:</span> <span className="text-[var(--foreground)]">{session.notes}</span></p>
          )}
        </div>
      )}
    </div>
