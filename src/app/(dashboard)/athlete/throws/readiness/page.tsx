...
export default function AthleteProfilePage() {
  ...
  const [checkInData, setCheckInData] = useState({
    ...
    sleepQuality: 3,
    energy: 5,
    soreness: { shoulder: 0, back: 0, hip: 0, knee: 0, elbow: 0, wrist: 0, general: 0 } as Record<
      string,
      number
    >,
    notes: "",
  ...
  const selfFeelingAccuracy =
    (p?.typing?.selfFeelingAccuracy as "accurate" | "moderate" | "poor") || "moderate";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-700 dark:text-surface-300">
            Readiness, recovery, and athlete typing
          </p>
        </div>
        <Link
          href="/athlete/throws"
        ...
        <>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-[var(--foreground)] mt-0.5">
                    Complete your Athlete Typing Quiz
                  </p>
                  <p className="text-xs text-surface-700 dark:text-surface-300 mt-0.5">
                    Your coach assigned this quiz. Takes ~5 minutes — tap to start.
                  </p>
                </div>
                <svg
          ...
          {podiumProfile &&
            (() => {
              ...
              const hasDeficit =
                deficitType && deficitType !== "none" && deficitType !== "balanced";
              ...
              return (
                <div className="card !p-4 space-y-3">
                  ...
                  {deficitType && deficitType !== "none" ? (
                    <div
                      className={`rounded-xl p-3 space-y-1.5 ${deficitLevel ? DEFICIT_LEVEL_BG[deficitLevel] : "bg-[var(--muted-bg)]/50"}`}
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-xs font-bold uppercase tracking-wide ${deficitLevel ? DEFICIT_LEVEL_COLORS[deficitLevel] : "text-surface-700 dark:text-surface-300"}`}
                        >
                          {hasDeficit ? "Training Focus" : "Status"}
                        </span>
                        <span
                          className={`text-xs font-medium ${deficitLevel ? DEFICIT_LEVEL_COLORS[deficitLevel] : "text-surface-700 dark:text-surface-300"}`}
                        >
                    ...
                    <p className="text-xs text-muted italic">
                      Your coach hasn&apos;t recorded test data yet. Check back after your next
                      testing session.
                    </p>
                  )}
                </div>
          ...
          <div className="card !p-4 space-y-3">
            ...
            {p.prs.length > 0 ? (
              <div className="flex gap-3 flex-wrap">
                {p.prs.map((pr, i) => (
                  <span
                    ...
                    style={{ backgroundColor: EVENTS[pr.event as ThrowEvent]?.color || "#666" }}
                  >
              ...
              <p className="text-xs text-muted italic">
                No PRs recorded yet. Tap &quot;Record PR&quot; to add one.
              </p>
            )}
            {showAddPR && (
            ...
            <div className="card !p-5 space-y-5">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCheckInData({ ...checkInData, energy: parseInt(e.target.value) })
                  }
                ...
                <div className="flex justify-between text-[10px] text-muted">
                  ...
                  <span className="font-bold text-surface-700 dark:text-surface-300">
                    {checkInData.energy} —{" "}
                    {ENERGY_SCALE.find((e) => e.value === checkInData.energy)?.label}
                  </span>
                  <span>Peak</span>
                </div>
              </div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCheckInData({
                              ...checkInData,
                              soreness: {
                                ...checkInData.soreness,
                                [zone.key]: parseInt(e.target.value),
                              },
                            })
                          }
                          className="w-full accent-primary-500"
          ...
          {p.typing?.quizCompletedDate && (
            <div className="card !p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-surface-700 dark:text-surface-300">Adaptation:</span>{" "}
                  <span className="font-semibold text-[var(--foreground)]">
                    Group {p.typing.adaptationGroup}
                  </span>
                </div>
                <div>
                  <span className="text-surface-700 dark:text-surface-300">Transfer:</span>{" "}
                  <span className="font-semibold text-[var(--foreground)] capitalize">
                    {p.typing.transferType?.replace(/-/g, " ")}
                  </span>
                </div>
                <div>
                  <span className="text-surface-700 dark:text-surface-300">Recovery:</span>{" "}
                  <span className="font-semibold text-[var(--foreground)] capitalize">
                    {p.typing.recoveryProfile}
                  </span>
                </div>
                <div>
                  <span className="text-surface-700 dark:text-surface-300">Method:</span>{" "}
                  <span className="font-semibold text-[var(--foreground)] capitalize">
                    {p.typing.recommendedMethod?.replace(/-/g, " ")}
                  </span>
                </div>
              </div>
            </div>
