...
export default function AthleteTypingQuizPage() {
  ...
  const isLastQuestion =
    currentQuizIdx === QUIZ_ORDER.length - 1 && currentQuestionIdx === questions.length - 1;
    ...
    if (currentQuestionIdx < questions.length - 1) {
      ...
      try {
        ...
        if (!res.ok || !data?.success) {
          throw new Error(data?.error || `Quiz submit failed (${res.status})`);
        }
        setDone(true);
      } catch (err) {
      ...
      <div className="animate-fade-in space-y-6">
        <div className="card text-center py-12 space-y-4">
          ...
          <p className="text-sm text-surface-700 dark:text-surface-300 max-w-xs mx-auto">
            Your Bondarchuk athlete type has been determined. Your coach will use this to
            personalise your training program.
          </p>
          <button onClick={() => router.push("/athlete/throws/readiness")} className="btn-primary">
            View My Profile
          </button>
        </div>
      </div>
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-xs text-muted">
          <span>
            {answeredSoFar} of {totalQuestions} questions answered
          </span>
      ...
      <p className="text-center text-xs text-muted">
        Answer honestly — there are no right or wrong answers. Your coach uses this to optimise your
        training.
      </p>
    </div>
  );
