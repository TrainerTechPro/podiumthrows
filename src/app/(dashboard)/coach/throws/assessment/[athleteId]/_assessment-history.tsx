...
export function AssessmentHistory({ assessments }: { assessments: AssessmentHistoryItem[] }) {
  ...
  return (
    <>
      <div className="card divide-y divide-[var(--card-border)]">
        {assessments.map((a) => {
          const resultsObj = a.results as Record<
            string,
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted">{exerciseCount} exercises</span>
                </div>
              ...
              {a.notes && <p className="text-xs text-muted mt-1 truncate">{a.notes}</p>}
            </button>
          );
        })}
      </div>
      ...
      {/* Detail Modal */}
      <Modal open={!!viewing} onClose={() => setViewId(null)} title="Assessment Detail">
        {viewing && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Badge
                variant={
                  (TYPE_COLORS[viewing.athleteType] as
                    | "warning"
                    | "primary"
                    | "success"
                    | "danger") ?? "neutral"
                  ...
                  {Object.entries(
                    ...
                    <div key={id} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-nano text-muted">{ex.category}</span>
                      </div>
                      <span
                        className={[
