    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
      {SORENESS_ZONES.map((zone) => {
        ...
        return (
          ...
          >
            <p className="text-[10px] font-bold text-[var(--foreground)] uppercase">{zone.label}</p>
            <p className="text-lg font-bold text-[var(--foreground)] font-mono">
              {val}
              <span className="text-[10px] text-surface-700 dark:text-surface-300 font-sans">
                /10
              </span>
            </p>
...
function CheckInForm({ athleteId, onSaved }: { athleteId: string; onSaved: () => void }) {
        ...
        <div className="flex gap-2">
          {SELF_FEELING_SCALE.map((s) => (
            ...
            >
              ...
              <span
                className={`block text-[9px] leading-tight mt-0.5 ${selfFeeling === s.value ? "text-white/80" : "text-muted"}`}
              >
                {s.label}
              </span>
            </button>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input
            ...
            step={0.5}
            value={sleepHours}
            onChange={(e) => setSleepHours(parseNumericInput(e.target.value) ?? 0)}
          ...
          <div className="flex gap-1">
            {SLEEP_QUALITY_SCALE.map((sq) => (
              ...
              >
                ...
                <span
                  className={`block text-[9px] leading-tight mt-0.5 ${sleepQuality === sq.value ? "text-white/80" : "text-muted"}`}
                >
                  {sq.label}
                </span>
              </button>
      ...
      <div className="space-y-1">
        ...
        <div className="flex justify-between text-[10px] text-muted">
          ...
          <span className="font-bold text-surface-700 dark:text-surface-300">
            {energy} — {ENERGY_SCALE.find((e) => e.value === energy)?.label}
          </span>
          <span>Peak</span>
        </div>
      </div>
...
export default function AthleteProfilePage() {
  ...
  const selfFeelingAccuracy =
    (profile?.typing?.selfFeelingAccuracy as "accurate" | "moderate" | "poor") || "moderate";
  ...
  const complexExercises: string[] = (() => {
    try {
      return JSON.parse(currentComplex?.exercises || "[]");
    } catch {
      return [];
    }
  })();
  const transferResult =
    primaryEventCode && primaryPR
  ...
  const qualification =
    primaryEventCode && primaryPR
      ...
      : "low";
  const volumeTargets = primaryEventCode
    ? ANNUAL_VOLUME_TARGETS[primaryEventCode]?.[qualification]
    : null;
        ...
        <div className="card text-center py-12 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-surface-700 dark:text-surface-300 mt-1">
              Add an athlete to your roster to get started.
            </p>
          </div>
          <Link
      ...
      {selectedAthleteId && error && !loading && (
        <div className="card text-center py-12 space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-surface-700 dark:text-surface-300 mt-1">
              There was a problem loading this athlete&apos;s profile data.
            </p>
          </div>
          <button
        ...
        <>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-0.5">
                    {profile.athlete.gender && (
                      <span className="text-[10px] text-surface-700 dark:text-surface-300 uppercase font-medium">
                        {profile.athlete.gender}
                      </span>
                    )}
                    ...
                    {profile.athlete.weight && (
                      <span className="text-[10px] text-surface-700 dark:text-surface-300">
                        {profile.athlete.weight}kg
                      </span>
                    )}
                    {profile.athlete.height && (
                      <span className="text-[10px] text-surface-700 dark:text-surface-300">
                        {profile.athlete.height}cm
                      </span>
                    )}
                  </div>
          ...
          {!profile.typing?.quizCompletedDate && (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                >
                  <svg
                    className={`w-5 h-5 ${profile.typing?.quizAssignedByCoach || quizAssigned ? "text-amber-600 dark:text-amber-400" : "text-red-600 dark:text-red-400"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p
                      className={`text-sm font-bold ${profile.typing?.quizAssignedByCoach || quizAssigned ? "text-amber-800 dark:text-amber-300" : "text-red-800 dark:text-red-300"}`}
                    >
                  ...
                  <p
                    className={`text-xs mt-0.5 ${profile.typing?.quizAssignedByCoach || quizAssigned ? "text-amber-700 dark:text-amber-400" : "text-red-700 dark:text-red-400"}`}
                  >
                    {profile.typing?.quizAssignedByCoach || quizAssigned
          ...
          {transferResult.exercises.length > 0 && (
            <div className="card !p-4 space-y-2">
              ...
              <p className="text-[10px] text-surface-700 dark:text-surface-300">
                Expected correlation coefficients from Bondarchuk Vol IV for current complex
                exercises
              </p>
              <div className="space-y-1">
                {transferResult.exercises.map((ex) => (
                  <div
                    key={ex.name}
                    className="flex items-center justify-between py-1 border-b border-[var(--card-border)] last:border-0"
                  >
                    <span className="text-xs text-surface-700 dark:text-surface-300 truncate max-w-[60%]">
                      {ex.name}
                    </span>
                    <div className="flex items-center gap-2">
                      ...
                      {primaryPR && (
                        <span>
                          ...
                          <strong className="text-[var(--foreground)]">
                            {primaryPR.distance.toFixed(2)}m
                          </strong>
                        </span>
                      )}
                      {compCount > 0 && (
                        <span className="flex items-center gap-1">
                          ...
                          />
                          Competition:{" "}
                          <strong className="text-[var(--foreground)]">{compCount}</strong>
                        </span>
                      )}
                      {trainCount > 0 && (
                        <span className="flex items-center gap-1">
                          <span
                            className="w-2 h-2 rounded-sm"
                            style={{ backgroundColor: eventColor, opacity: 0.7 }}
                          />
                          Training:{" "}
                          <strong className="text-[var(--foreground)]">{trainCount}</strong>
                        </span>
                      )}
                    </div>
                  </div>
            ...
            {profile.typing ? (
              <div className="space-y-2">
                {[
                  {
                    label: "Adaptation",
                    value: profile.typing.adaptationGroup
                      ? `Group ${profile.typing.adaptationGroup} — ${profile.typing.adaptationGroup === 1 ? "Fast" : profile.typing.adaptationGroup === 2 ? "Moderate" : "Slow"}`
                      : "—",
                    confidence: profile.typing.confidenceAdaptation,
                  },
                  {
                    label: "Transfer",
                    value:
                      profile.typing.transferType
                        ?.replace(/-/g, " ")
                        .replace(/\b\w/g, (c: string) => c.toUpperCase()) || "—",
                    confidence: profile.typing.confidenceTransfer,
                  },
                  {
                    label: "Self-Feeling",
                    value:
                      profile.typing.selfFeelingAccuracy?.replace(/\b\w/g, (c: string) =>
                        c.toUpperCase()
                      ) || "—",
                    confidence: profile.typing.confidenceSelfFeeling,
                  },
                  {
                    label: "Light Impl",
                    value:
                      profile.typing.lightImplResponse === "normal-87pct"
                        ? "Normal (87% group)"
                        : profile.typing.lightImplResponse === "tolerant-13pct"
                          ? "Tolerant (13% group)"
                          : "—",
                    confidence: null,
                  },
                  {
                    label: "Recovery",
                    value:
                      profile.typing.recoveryProfile?.replace(/\b\w/g, (c: string) =>
                        c.toUpperCase()
                      ) || "—",
                    confidence: null,
                  },
                  {
                    label: "Method",
                    value:
                      profile.typing.recommendedMethod
                        ?.replace(/-/g, " ")
                        .replace(/\b\w/g, (c: string) => c.toUpperCase()) || "—",
                    confidence: null,
                  },
                ...
                <div className="pt-2 flex items-center gap-4 text-[10px] text-surface-700 dark:text-surface-300">
                  <span>
                    Duration: <strong>{profile.typing.optimalComplexDuration || "—"}</strong>
                  </span>
                  <span>
                    Sessions to form:{" "}
                    <strong>~{profile.typing.estimatedSessionsToForm || "—"}</strong>
                  </span>
                </div>
              </div>
            ) : (
          ...
          <div className="card !p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {todayCheckin ? (
                  <>
                    <span className="text-[10px] text-muted">Filled today</span>
                    <SourceBadge source={todayCheckin.source} />
                  </>
                ) : profile.checkins.length > 0 ? (
                  <span className="text-[10px] text-muted">Last: {profile.checkins[0].date}</span>
                ) : null}
              </div>
            </div>
            {todayCheckin ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="text-center">
                    <p className="text-[10px] text-surface-700 dark:text-surface-300 uppercase">
                      Self-Feeling
                    </p>
                    <p className="text-2xl font-bold text-[var(--foreground)] font-mono">
                      {todayCheckin.selfFeeling}/5
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] text-surface-700 dark:text-surface-300 uppercase">
            ...
            {profile.assignments.length > 0 ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-3 text-[10px] text-surface-700 dark:text-surface-300">
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-[#5BB88A]" /> Completed
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-[#D4915A]" /> Partial
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-[#D46A6A]" /> Skipped
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-[#CBD5E1]" /> Assigned
                  </span>
                </div>
              </div>
            ) : (
          ...
          <div className="card !p-4 space-y-3">
            ...
            {volumeTargets ? (
              <div className="space-y-3">
                <p className="text-[10px] text-surface-700 dark:text-surface-300">
                  {primaryEvent ? EVENTS[primaryEvent]?.label : "Throws"} —{" "}
                  {qualification.toUpperCase()} qualification level
                </p>
                {(["competition", "lighter", "heavier", "analogous", "total"] as const).map(
                  (cat) => {
                    ...
                    return (
                      <div key={cat} className="space-y-1">
                        <div className="flex justify-between text-[10px]">
                          <span className="text-surface-700 dark:text-surface-300 capitalize font-medium">
                            {cat}
                          </span>
          ...
          <div className="card !p-4 space-y-3">
            ...
            {nextComp ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-lg font-bold text-[var(--foreground)]">{daysToComp} days</p>
                ...
                <div className="p-2 rounded-lg bg-[var(--muted-bg)]/50">
                  {adaptation.phase === "in-form" ? (
                    ...
                    <p className="text-xs text-yellow-600 dark:text-yellow-400 font-semibold">
                      ...
                      {Math.max(
                        0,
                        (profile.typing?.estimatedSessionsToForm || 18) -
                          (currentComplex?.sessionsCount || 0)
                      )}{" "}
                      sessions remaining (est.)
                    </p>
                  ) : (
                    <p className="text-xs text-surface-700 dark:text-surface-300">
                ...
                {taper && daysToComp != null && daysToComp <= 7 && (
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-surface-700 dark:text-surface-300">
                      Volume at <strong>{Math.round(taper.volumeMultiplier * 100)}%</strong> (
                      {daysToComp} days out)
                    </p>
                  </div>
                )}
                ...
                <p className="text-[10px] text-surface-700 dark:text-surface-300">
                  Event: {EVENTS[nextComp.event as ThrowEvent]?.label || nextComp.event} | Date:{" "}
                  {nextComp.date}
                </p>
              </div>
            ) : (
  ...
  return (
    <div className="card !p-5 space-y-4">
      ...
      {drillPRs.length === 0 && !showForm && (
        <p className="text-sm text-muted text-center py-4">No drill PRs logged yet.</p>
      )}
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <span className="text-xs text-muted w-12 shrink-0">{pr.implement}</span>
...
const TESTING_FIELDS: Record<string, TestingField[]> = {
  ...
  Power: [
    {
      key: "overheadMB",
      label: "Overhead Med Ball",
      metricUnit: "m",
      imperialUnit: "ft",
      imperialFactor: 3.28084,
      step: "0.01",
    },
    {
      key: "rotationalMB",
      label: "Rotational Med Ball",
      metricUnit: "m",
      imperialUnit: "ft",
      imperialFactor: 3.28084,
      step: "0.01",
...
function TestingMetricsPanel({
 ...
 text-surface-700 dark:text-surface-300"
        ...
        <p className="text-sm text-muted text-center py-4">No benchmarks recorded yet.</p>
      ) : (
        <div className="space-y-4">
          {Object.entries(TESTING_FIELDS).map(([category, fields]) => {
              ...
              <div key={category}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {populated.map((field) => {
                    ...
                    const imperialVal =
                      field.imperialFactor != null
                        ? round(metricVal * field.imperialFactor, 1)
                        : null;
                    return (
                      <div key={field.key} className="bg-[var(--muted-bg)]/50 rounded-lg px-3 py-2">
...
function RecordPRForm({ athleteId, onSaved }: { athleteId: string; onSaved: () => void }) {
...
function ManageComplexForm({
  ...
  function addCustom() {
    ...
    if (!trimmed || selectedExercises.includes(trimmed)) {
      setCustomExercise("");
      return;
    }
          ...
          {currentComplex ? (
            <>
              ...
              <strong className="text-surface-700 dark:text-surface-300">
                {currentComplex.startDate}
              </strong>{" "}
              · {currentComplex.sessionsCount} sessions
            </>
          ) : (
            "No active complex"
          )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="text-xs font-semibold text-surface-700 dark:text-surface-300">
          Strength Exercises{" "}
          <span className="font-normal text-muted">(select all in this complex)</span>
        </label>
      ...
      <div className="space-y-1">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={addCustom} className="btn-secondary text-xs px-3 py-2">
            Add
          </button>
        </div>
      </div>
      {selectedExercises.length > 0 && (
        <div className="space-y-1">
          <p className="text-[10px] text-surface-700 dark:text-surface-300 font-semibold uppercase">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => setExpanded(false)} className="btn-secondary text-xs px-3 py-2">
          Cancel
        </button>
      </div>
    </div>
...
function AddCompetitionForm({ athleteId, onSaved }: { athleteId: string; onSaved: () => void }) {
...
function CompetitionsPanel({
  ...
  return (
    <div className="card !p-5 space-y-4 border-2 border-purple-200 dark:border-purple-800">
      ...
      {upcoming.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-wider text-surface-700 dark:text-surface-300">
            Upcoming
          </p>
          {upcoming.map((c) => (
            <div
              key={c.id}
              className="flex items-center justify-between py-2 border-b border-[var(--card-border)] last:border-0 gap-2 flex-wrap"
            >
              <div>
                <p className="text-xs font-semibold text-[var(--foreground)]">{c.name}</p>
                <p className="text-[10px] text-surface-700 dark:text-surface-300">
                  {c.date} · {EVENTS[c.event as ThrowEvent]?.label || c.event} · Priority{" "}
                  {c.priority}
                </p>
              </div>
              <div className="flex items-center gap-2">
                {c.result != null ? (
                  <div className="flex items-center gap-1">
                    <span className="text-xs font-mono font-bold text-[var(--foreground)]">
                      {c.result.toFixed(2)}m
                    </span>
                    {c.resultBy && <SourceBadge source={c.resultBy} />}
                    <button
                      onClick={() => {
                        setEditingId(c.id);
                        setResultInput(c.result!.toString());
                      }}
...
function TypingOverrideForm({
  ...
  const [adaptationGroup, setAdaptationGroup] = useState(
    current?.adaptationGroup?.toString() || ""
  );
  const [transferType, setTransferType] = useState(current?.transferType || "");
  const [selfFeelingAccuracy, setSelfFeelingAccuracy] = useState(
    current?.selfFeelingAccuracy || ""
  );
  ...
  const inputCls =
    "w-full px-3 py-2 rounded-lg border border-[var(--card-border)] bg-[var(--card-bg)] text-sm text-[var(--foreground)]";
  ...
  return (
    <div className="p-3 rounded-lg bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800 space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-[10px] font-semibold text-surface-700 dark:text-surface-300 uppercase">
            Adaptation Group
          </label>
          <select
            value={adaptationGroup}
            onChange={(e) => setAdaptationGroup(e.target.value)}
            className={inputCls}
          >
            <option value="">Not set</option>
        ...
        <div className="space-y-1">
          ...
          <select
            value={selfFeelingAccuracy}
            onChange={(e) => setSelfFeelingAccuracy(e.target.value)}
            className={inputCls}
          >
            <option value="">Not set</option>
        ...
        <div className="space-y-1">
          ...
          <select
            value={lightImplResponse}
            onChange={(e) => setLightImplResponse(e.target.value)}
            className={inputCls}
          >
            <option value="">Not set</option>
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) => setRecoveryProfile(e.target.value)}
            className={inputCls}
          >
            <option value="">Not set</option>
...
function PRProgressionChart({ athleteId }: { athleteId: string }) {
  ...
  const filteredPRs = prHistory
    .filter(
      (p) => p.event === selectedEvent && (!selectedImplement || p.implement === selectedImplement)
    )
  ...
  const chartData = allDates.map((date) => {
    ...
    const benchPoint = overlay
      ? benchHistory.filter((b) => b.recordedAt <= date).slice(-1)[0]
      : null;
