  ...
  if (!athleteId) {
    return (
      <div className="animate-spring-up card text-center py-12">
        <p className="text-muted">No athlete selected. Go back and select an athlete.</p>
      </div>
    );
  }
  ...
  async function handleSelect(score: Record<string, number>) {
    ...
    if (currentQuestionIdx < questions.length - 1) {
      ...
      try {
        ...
        // Allow retry
        logger.debug("Allow retry", {
          context: "src/app/(dashboard)/coach/throws/profile/typing/page.tsx",
    ...
    return (
      <div className="animate-spring-up space-y-6">
        <div className="card text-center py-12 space-y-4">
          ...
          <h2 className="text-xl font-bold text-[var(--foreground)]">Typing Complete</h2>
          <p className="text-sm text-surface-700 dark:text-surface-300">
            The athlete&apos;s Bondarchuk typing has been saved. Classification will refine
            automatically as training data accumulates.
          </p>
          <button
  ...
  if (saving) {
    return (
      <div className="animate-spring-up card text-center py-12">
        ...
        <p className="text-muted">Saving typing results...</p>
      </div>
    );
  }
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option) => (
            <button
              ...
              className="w-full text-left p-4 rounded-xl border border-[var(--card-border)] hover:border-primary-500 dark:hover:border-primary-500 hover:bg-[rgba(212,168,67,0.08)] transition-all duration-150 group"
            >
              <span className="text-sm font-medium text-[var(--foreground)] group-hover:text-primary-600 dark:group-hover:text-primary-500">
                {option.label}
              </span>
            </button>
          ))}
