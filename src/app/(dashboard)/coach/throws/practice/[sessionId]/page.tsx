...
interface LogAttemptPanelProps {
  ...
  queueAttempt: (
    sessionId: string,
    payload: import("@/lib/pwa/sync-queue").AttemptPayload
  ) => Promise<import("@/lib/pwa/sync-queue").QueuedAttempt>;
}

function LogAttemptPanel({
  athlete,
  sessionId,
  athleteAttemptCount,
  onSave,
  onCancel,
  isOnline,
  queueAttempt: queueAttemptFn,
}: LogAttemptPanelProps) {
  const defaultEvent = athlete.throwsProfile?.event ?? "SHOT_PUT";
  const [event, setEvent] = useState(defaultEvent);
  const [implement, setImplement] = useState(() =>
    getDefaultImplement(athlete.throwsProfile, defaultEvent)
  );
  ...
  const distanceNum =
    !isNaN(distanceRaw) && distanceUnit === "feet" ? distanceRaw * 0.3048 : distanceRaw;
  const wouldBePR =
    !isNaN(distanceNum) &&
    distance !== "" &&
  ...
  function handleVideoSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    vid.onloadedmetadata = () => {
      ...
      if (dur > MAX_VIDEO_SECONDS + 0.05) {
        setVideoError(
          `Clip is ${dur.toFixed(1)}s — trim to ${MAX_VIDEO_SECONDS}s on your iPhone before uploading.`
        );
        setVideoFile(null);
      } else {
        setVideoFile(file);
    ...
    try {
      ...
      if (videoFile) {
        ...
        const uploadRes = await fetch("/api/throws/practice/video-upload", {
          ...
          headers:
            videoDuration !== null
              ? { "x-video-duration": String(videoDuration), ...csrfHeaders() }
              : { ...csrfHeaders() },
          body: fd,
        });
        ...
        <div className="sticky top-0 bg-[var(--surface-overlay)] px-5 pt-5 pb-3 border-b border-[var(--card-border)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Attempt #{athleteAttemptCount + 1}</p>
            </div>
          </div>
          <button
            onClick={onCancel}
...
export default function LiveSessionPage() {
  ...
  const { pendingCount, isSyncing, queueAttempt } = useSyncQueue((results) => {
  ...
  function handleAttemptDeleted(attemptId: string) {
    // Optimistic removal
    setSession((prev) =>
      prev ? { ...prev, attempts: prev.attempts.filter((a) => a.id !== attemptId) } : prev
    );
    // Confirm on server
    fetch(`/api/throws/practice/${params.sessionId}/attempts/${attemptId}`, {
      method: "DELETE",
      headers: csrfHeaders(),
    });
  }
  ...
  for (const attempt of session.attempts) {
    ...
    const attemptAthlete = attempt.athlete as unknown as {
      throwsPRs?: { event: string; implement: string; distance: number }[];
    };
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {rosterAthletes.length === 0 ? (
            ...
            rosterAthletes.map((athlete) => {
              ...
              const eventLabel =
                EVENT_LABELS[athlete.throwsProfile?.event ?? ""] ??
                athlete.throwsProfile?.event ??
                "—";
        ...
        <div className="lg:col-span-3 space-y-3">
          <h2 className="text-sm font-semibold text-surface-700 dark:text-surface-300">
            ...
            {session.attempts.length > 0 && (
              <span className="ml-1.5 text-muted font-normal">({session.attempts.length})</span>
            )}
          </h2>
          {session.attempts.length === 0 ? (
            <div className="card text-center py-12 space-y-2">
              <svg
                className="w-8 h-8 text-[var(--color-border-strong)] mx-auto"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-xs text-muted">
                {isClosed
                  ...
                  : "Tap an athlete to log their first attempt."}
              </p>
            </div>
          ) : (
            // Show newest first
