...
// Dev fallback must match the node-runtime fallback in src/lib/auth.ts —
// API signs with that default; middleware verifies here. A mismatch silently
// bounces every dev login back to /login (we've shipped that bug twice).
