...
| `/athlete/quick-start`                        | Smart-routing "Start Session" entry point used by `src/components/ui/QuickActions.tsx:57`. Priority ladder: in-progress throws assignment → next self-program session → pending coach assignment → ad-hoc log-session. Flattening to `/athlete/log-session` would break the resume-workout path. **Status:** keep as-is. If we consolidate later, the logic moves into `/athlete/log-session/page.tsx` as a server-side branch, not a client redirect. |
