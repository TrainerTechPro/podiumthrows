...
## 9. PR 6 follow-ups (coach-IA consolidation, tracked)

Work deferred during the PR 6 commit chain (`b120d62..f95ce4c`, 13 commits, 2026-04-25). Not gating future PRs. Each entry names the concrete next action.

- **Throw-profile invite CTA on `/coach/athletes/invitations`** — pull the throw-profile invitation flow from the legacy `/coach/throws/invite` (342 lines at `src/app/(dashboard)/coach/throws/invite/page.tsx`) into the Tier-2 Invitations sibling route as a "Send Throw Profile" CTA + side-panel. Once landed, redirect `/coach/throws/invite` → `/coach/athletes/invitations` in `next.config.mjs` and add the entry to `e2e/coach-ia-redirects.spec.ts`. Real user value, ~1 hour. Deferred from `commit 6c` (`83c1b1e`) to keep that commit scoped.
- **Inline extract `<ThrowsProfilePage>`** (3334 lines, `useSearchParams`-coupled) — refactor `src/app/(dashboard)/coach/throws/profile/page.tsx` so the working component accepts `athleteId` as a prop with searchParams fallback, then have `/coach/athletes/[id]/profile/page.tsx` render it inline instead of redirecting to the legacy URL. The redirect-shell from commit 6c is the swap point. Internal hygiene only — invisible to users; defer until something else forces a touch on that component.
